...
export class UiGridComponent<T extends IGridDataEntry> extends ResizableGrid<T> implements AfterContentInit, OnChanges, OnDestroy {
     ...
     *
     * @param row row in which this message needs to appear
     */
    public showGroupMessage(row: T, last: boolean): boolean {
        ...
        this.groupMessage = (column.groupMessageCb && column.groupMessageCb(propertyValue)) || propertyValue;
    ...
    public toggleGroupCollapse(row: T) {
        const column = this.sortManager.groupedBy as UiGridColumnDirective<T>;
        const propertyValue = this.dataManager.getProperty(row, column.property!);
        const groupMessage = (column.groupMessageCb && column.groupMessageCb(propertyValue)) || propertyValue;
